...
class iNaturalist(Dataset):
    def __init__(
        self, root, ann_file, is_train=True, transform=None, target_transform=None
    ):
        ...
        self.taxonomy, self.classes_taxonomic = load_taxonomy(
            ann_data, self.tax_levels, self.labels
        )
...
def truncate_and_resplit_dataset(
    train_dataset,
    val_dataset,
    test_dataset=None,
    num_test_samples=100,
    frac_val=0.1,
    return_label_arrays=False,
    print_info=True,
    seed=0,
):
    """
    ...
    if print_info:
        print(
            f"Kept {len(kept_classes)} classes after filtering out classes with less than {num_test_samples+1} examples\n"
        )
    ...
    if dataset_name == "plantnet":
        ...
        if truncate:
            ...
            if os.path.isfile(pth):
                ...
                _, _, _, label_remapping = truncate_and_resplit_dataset(
                    train_dataset,
                    val_dataset,
                    test_dataset,
                    num_test_samples=100,
            ...
            train_dataset = PlantNet(
                root,
                "train",
                transform=transform_train,
                target_transform=target_transform,
            )
            val_dataset = PlantNet(
                root, "val", transform=transform_test, target_transform=target_transform
            )
            test_dataset = PlantNet(
                root,
                "test",
            ...
            output = truncate_and_resplit_dataset(
                train_dataset,
                val_dataset,
                test_dataset,
                num_test_samples=100,
        ...
        if truncate:
            ...
            if os.path.isfile(pth):
                ...
                full_train_dataset = iNaturalist(
                    dataset_root, train_annot, transform=transform
                )
...
def get_dataloaders(config, root=None):
    ...
    train_dataset, val_dataset, test_dataset = get_datasets(
        config["dataset_name"], truncate=config["truncate"], root=root
    )
    ...
    if config[
        "proper_cal"
    ]:  # Do 4-way datasplit by selecting a random 30% of val to become a proper validation set
...
def train_model(model, train_loader, val_loader, config):
    ...
    save_every_epoch = (
        True  # save weights every epoch if accuracy is better than previous best
    )
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for phase in ["train", "val"]:
            ...
            if config["use_last_epoch"]:
                ...
                torch.save(
                    model.state_dict(), cache_folder + config["model_filename"] + ".pth"
                )
...
def get_softmax_and_labels(dataloader, model, config):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(dataloader):
            inputs = inputs.to(
                config["device"]
            )  # Use GPU when available to speed up computation
            labels_arr[j : j + inputs.shape[0]] = labels.numpy()

            # Get model outputs
            softmax_arr[j : j + inputs.shape[0], :] = (
...
def get_cal_test_softmax_and_labels(config):
    ...
    if config["proper_cal"]:
        ...
        np.save(
            cache_folder + config["model_filename"] + "_cal_softmax.npy", cal_softmax
        )
        ...
        np.save(
            cache_folder + config["model_filename"] + "_val_softmax.npy", val_softmax
        )
    ...
    print(
        "Saved val/cal and test softmax+labels to "
        + cache_folder
    ...
    _, _, _, train_labels, _, _ = get_datasets(
        config["dataset_name"], truncate=config["truncate"], return_labels=True
    )
    if config["truncate"]:
...
def get_model(config):
    ...
    try:
        state_dict = torch.load(
            cache_folder + config["model_filename"] + ".pth",
        ...
        for setting in [
            ...
            )  # If the configs aren't equal, retrain
