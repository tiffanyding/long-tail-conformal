    ...
    for alpha in alphas:
        ...
        for method in methods_found:
            ...
            if method in plot_data["plot_points"]["MarginalCov"][alpha]:
                ...
                all_res[alpha_key][method]["coverage_metrics"][
                    "marginal_cov"
                ] = x_marginal
            ...
            if method in plot_data["plot_points"]["At-risk avg"][alpha]:
                ...
                x_not_at_risk, _ = plot_data["plot_points"]["Not-at-risk avg"][alpha][
                    method
                ]
...
def load_complete_analysis_data(dataset_name, alphas, methods):
    ...
    for method in methods:
        try:
            test_result = _load_one_result_internal(
                dataset_name, 0.1, method, score="softmax"
            )
    ...
    all_res = _load_all_results_internal(
        dataset_name, alphas, available_methods, score="softmax"
    )
    ...
    all_res = _compute_wpas_results_internal(
        all_res, at_risk_species, num_classes, dataset_name
    )
...
def _load_one_result_internal(
    dataset,
    alpha,
    method_name,
    score="softmax",
    train_class_distr=None,
    test_labels=None,
):
    ...
    if (train_class_distr is not None) and (test_labels is not None):
        metrics["set_size_metrics"]["train_mean"] = (
            compute_train_weighted_average_set_size(
                dataset, metrics, train_class_distr, test_labels
            )
        )
...
def load_one_result(
    ...
    return _load_one_result_internal(
        dataset, alpha, method_name, score, train_class_distr, test_labels
    )
    ...
    for alpha in alphas:
        ...
        for method in methods:
            if dataset.endswith("-trunc"):  # Compute train-weighted average set size
                res[method] = _load_one_result_internal(
                    dataset,
                    alpha,
                    method,
                    score=score,
                ...
                res[method] = _load_one_result_internal(
                    dataset, alpha, method, score=score
                )
...
def _compute_wpas_results_internal(all_res, at_risk_species, num_classes, dataset_name):
    ...
    for gamma in gammas:
        ...
        cal_scores = get_conformal_scores(
            cal_softmax, "WPAS", train_labels_path=train_labels_path, weights=weights
        )
        test_scores = get_conformal_scores(
            test_softmax, "WPAS", train_labels_path=train_labels_path, weights=weights
        )
        ...
        for alpha in alphas:
            ...
            coverage_metrics, set_size_metrics = compute_all_metrics(
                test_labels, preds, alpha
            )
...
def _get_plantnet_at_risk_species_internal():
    ...
    df = pd.DataFrame.from_dict(
        names_as_numbers, orient="index", columns=["species_id"]
    )
    ...
    # Add IUCN status
    df_iucn = pd.DataFrame.from_dict(
        status_iucn, orient="index", columns=["iucn_status"]
    )
...
def plot_results(
    all_res, at_risk_species, alphas, num_classes, dataset, fast_mode=False
):
        ...
        for j, alpha in enumerate(alphas):
            ...
            if alpha_key in all_res:
                ...
                for score in display_methods:
                    if score in all_res[alpha_key]:
                        ...
                        if i == 0:  # Avg of at risk
                            x = np.mean(
                                res["coverage_metrics"]["raw_class_coverages"][
                                    at_risk_species
                                ]
                            )
                        ...
                        if display_label is not None:
                            ...
                            total_width = (
                                22  # ex units (reduced for more compact spacing)
                            )
                        ...
                        if score.startswith("WPAS"):
                            ...
                            alpha_transparency = alpha_to_transparency.get(
                                alpha, 1
                            )  # Default to 0.6 if alpha not found
                        else:
                            alpha_transparency = (
                                1  # Default transparency for non-WPAS methods
                            )
                ...
                if len(wpas_data) > 1:
                    ...
                    line_alpha = (
                        alpha_to_transparency.get(alpha, 0.5) * 0.7
                    )  # Slightly more transparent for line
    ...
    legend_standalone = legend_fig.legend(
        handles, labels, ncol=len(alphas), loc="center", fontsize=12, frameon=True
    )
    ...
    print(
        "   - Standard w. PAS: Standard with prevalence-adjusted scoring (orange triangle, larger marker)"
    )
    print(
        "   - Standard w. WPAS: Weighted prevalence-adjusted scoring with γ=1,10,100,1000 (green circles connected by lines)"
    )
    print(
...
def main():
    ...
    if check_simple_cache():
        ...
        all_res, at_risk_species, available_methods, num_classes, num_test_samples = (
            load_from_simple_cache()
        )
        print(
            f"✓ Dataset: {dataset} | Methods: {available_methods} | Classes: {num_classes}"
        )
    ...
    plot_results(
        all_res, at_risk_species, alphas, num_classes, dataset, fast_mode=fast_mode
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--clear-cache":
            ...
            if check_simple_cache():
                print(
                    f"Simple cache: {simple_cache_file} (found - enables ultra-fast mode)"
                )
            else:
            ...
            print("Usage:")
            print(
                "  python weighted_macro_coverage_plots.py          # Run with caching"
            )
            print(
                "  python weighted_macro_coverage_plots.py --fast         # Ultra-fast mode (no LaTeX)"
            )
            print(
                "  python weighted_macro_coverage_plots.py --clear-cache  # Clear cache and exit"
            )
            print(
                "  python weighted_macro_coverage_plots.py --cache-info   # Show cache information"
            )
            print(
                "  python weighted_macro_coverage_plots.py --help         # Show this help"
            )
            print(
                "\nNote: If plot_data_cache.pkl exists (from running weighted_macro_coverage_plots_simple.py),"
            )
            print(
                "      it will be used automatically for ultra-fast execution (~0.6 seconds)!"
            )
            exit(0)
    ...
    if fast_mode_override:
        ...
        def main():
            ...
            if cache_exists:
                ...
                print(
                    "🔄 First run - computing and caching data (will be fast next time)..."
                )

            alphas = [0.2, 0.1, 0.05, 0.01]
            methods = ["standard", "prevalence-adjusted"]

            analysis_data = load_complete_analysis_data(
                dataset, tuple(alphas), tuple(methods)
            )
            all_res = analysis_data["all_res"]
            at_risk_species = analysis_data["at_risk_species"]
            available_methods = analysis_data["available_methods"]
            ...
            plot_results(
                all_res, at_risk_species, alphas, num_classes, dataset, fast_mode=True
            )
