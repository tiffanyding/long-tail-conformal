...
def plot_qhats(method_qhats, method_label, dataset, alpha, folder, 
    ...
    ax.plot(method_qhats[idx], 'o', label=method_label, alpha=0.3, markersize=4, 
            color=color, markeredgewidth=0)
...
plot_qhats(method_qhats, 'Standard w. PAS', dataset, alpha, results_folder, color='gold',
...
for i, bw in enumerate(fuzzy_bandwidths):
    get_and_plot_qhats(f'fuzzy-rarity-{bw}', f'Raw Fuzzy ($\\sigma=$ {bw})',
                       dataset, alpha, results_folder, color='salmon',
...
cvx_weights = [1, .9, .75, .5, .25, 0]
for i, w in enumerate(cvx_weights):
    get_and_plot_qhats(f'cvx-cw_weight={w}', f'Interp-Q ($\\tau=$ {w})', 
...
handles = [
    Line2D([0], [0], color='blue', linestyle='-', linewidth=2, label='Standard $\\hat{q}$'),
    Line2D([0], [0], color='red',  linestyle='--', linewidth=2, label='Classwise $\\hat{q}_y^{\\mathrm{CW}}$'),
]
